...
namespace wrapper {
	...
	const inline void show_message(std::string msg) {
		MessageBoxA(nullptr, msg.c_str(), "[dbg] gasper", MB_OK);
	}
	...
	const inline bool handle_issue(const char* name, void* ptr)
	{
		...
		const auto s_formatting = "%s not intialized, value -> 0x%X";
		...
		if (!ptr)
		{
			...
			if (len <= 0)
				show_message("Issue formatting issue_handler wrapper::#handle_issue#26");
	...
	inline std::shared_ptr<c_context> create_gl_context()
	{
		...
		res->m_hdc_devicectx = GetDC(find_window("NationsGlory"));
